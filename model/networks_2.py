...
class kernel_residual_fc_embeds(nn.Module):
    def __init__(self, input_ch, intermediate_ch=512, grid_ch = 64, num_block=8, output_ch=1, grid_gap=0.25, grid_bandwidth=0.25, bandwidth_min=0.1, bandwidth_max=0.5, float_amt=0.1, min_xy=None, max_xy=None, probe=False):
        ...
        with open("D:/D/LNAF/3/Learning_Neural_Acoustic_Fields-master/apartment_1_distance", "rb") as f:
            self.grid_0 = torch.tensor(pickle.load(f), device="cuda:0")

        self.conv = nn.Conv1d(in_channels=1, out_channels=grid_ch, kernel_size=1)
    ...
    def forward(self, input_stuff, rot_idx, sound_loc=None):
        ...
        grid_0 = self.grid_0.unsqueeze(0).unsqueeze(0)
        grid_0 = self.conv(grid_0)
        grid_0 = grid_0.squeeze(0).transpose(0, 1)

        grid_feat_v0 = fit_predict_torch(grid_coors_baseline, grid_0, sound_loc_v0, self.bandwidths)
        grid_feat_v1 = fit_predict_torch(grid_coors_baseline, grid_0, sound_loc_v1, self.bandwidths)
